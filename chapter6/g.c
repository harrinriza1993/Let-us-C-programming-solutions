/*(g)The natural logaritham can be approximated by the following series.
...
Apporach:
	1)The value of n is first given as x-1/x, outside of the for loop.
	2)Inside the for loop the value n is given as 
	   natural_logarithm = natural_logarithm + 1/2(x-1/x)^power
	  to get the natural logaritham.
...
{
	int x;
	float natural_logarithm, power=1;

	printf("Enter the value of x\n");
	scanf("%d",&x);

	natural_logarithm = (x-1)/x;

	/*The value of power ranges from 2 to 7 since we need first
	seven terms of the series */
	for ( power = 2;power <= 7;power++)
	{
		//The natural logarithm is obtained using this formula given below.
